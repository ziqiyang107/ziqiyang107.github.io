...
$$
q_{t,t-h}(y|x) = \frac{q_{t-h,t}(x|y)q_{t-h}(y)}{q_t(x)},
$$
...
$$
\begin{align*}
p(y,t-h)&=p(x,t)+\nabla p(x,t)\cdot (y-x)-h\partial_t p(x,t)+O(|y-x|^2+h^2)  \\
&=p(x,t)+\sqrt{h}z\cdot \nabla p(x,t)+O(h)
\end{align*}
$$
where we assume the regularization on $p$ that $\partial_t p(x,t)$ is continuous, and hence locally bounded, can can be absorbed in $O(h)$. Then we divide above formula by $p(x,t)$ and exponentiate(using $1+\epsilon=e^{\epsilon+O(\epsilon^2)}$):
$$
\frac{p(y,t-h)}{p(x,t)}=exp\Big(\sqrt{h}z\cdot \nabla \log p(x,t)+O(h) \Big)
$$
...
$$
Since $y=x+\sqrt{h}z$, then conditional distribution of $X_{t-h}$ given $X_t=x$ is $\mathcal{N}\Big(x+h(\sigma^2 \log p -b ), \sigma^2 h \vecI \Big)$, then we let $h \rightarrow \infty$ and get the reverse time SDE
$$
dX_t = (\sigma^2 \nabla \log p_t(X)-b(X_t, t))dt + \sigma d\bar{W}_t,
$$
