...
& \sum_{t=2}^T E_{q(\vecx_t|\vecx_0)} [D_{KL}(q(\vecx_{t-1}|\vecx_t, \vecx_0)||p_{\theta}(\vecx_{t-1}|\vecx_t))] \numberthis \label{Eq: lower bound}
\end{align*}

This equation is almost the equatinon(5) in \cite{ho2020denoising}, but doesn't include the expectation over $\vecx_0$ and sign is reversed. We use the same definition as in the DDPM paper
...
$$
\text{Equation}\eqref{Eq: lower bound} = -E_{\vecx_1 \sim q(\vecx_1|\vecx_0)}[L_0] - L_T - \sum_{t=2}^T E_{\vecx_t \sim q(\vecx_t|\vecx_0)}[L_{t-1}]
$$
...
\textcolor{red}{Derive $q(\vecx_t\|\vecx_0)$:}

In order to derive equation (6)(7) in \cite{ho2020denoising}, we first need to find $q(\vecx_t\|\vecx_0)$. Using reparameterization trick, we can write $\vecx_t = \sqrt{\alpha_t}\vecx_{t-1}+\sqrt{1-\alpha_t}\veceps_{t-1}$, where $\veceps_{t-1} \sim N(0, \vecI)$, $\alpha_t:=1-\beta_t$. Similarly we expand $\vecx_{t-1}$ and get:
...
\textcolor{red}{Derive equation(6)(7) in }\cite{ho2020denoising}:
...
\textcolor{red}{Four equivalent approaches in simplifying $L_{t-1}$:}
...
$$
\begin{align*}
\tilde{\vecmu}_t(\vecx_t, \vecx_0) =& \frac{\sqrt{\bar{\alpha}_{t-1}}\beta_t}{1-\bar{\alpha}_t}\vecx_0 + \frac{\sqrt{\alpha_t}(1-\bar{\alpha}_{t-1})}{1-\bar{\alpha}_t}\vecx_t \numberthis \label{Eq:mu_t form x} \\
...
=& \frac{1}{\sqrt{\alpha_t}}\Big(\vecx_t - \frac{1-\alpha_t}{\sqrt{1-\bar{\alpha}_t}}\veceps_t \Big) \numberthis \label{Eq: mu_t form eps}
\end{align*}
$$
...
$$
\begin{align*}
\vecmu_{\theta}(\vecx_t, t) =& \frac{\sqrt{\bar{\alpha}_{t-1}}\beta_t}{1-\bar{\alpha}_t}\vecx_{\theta}(\vecx_t, t) + \frac{\sqrt{\alpha_t}(1-\bar{\alpha}_{t-1})}{1-\bar{\alpha}_t}\vecx_t \numberthis \label{Eq: Eq:mu_theta form x}  \\
=& \frac{1}{\sqrt{\alpha_t}}\Big(\vecx_t - \frac{1-\alpha_t}{\sqrt{1-\bar{\alpha}_t}}\veceps_{\theta}(\vecx_t, t) \Big) \numberthis \label{Eq: mu_theta form eps}
\end{align*}
$$
...
$$
\begin{align*}
&\text{If we plug formula \eqref{Eq:mu_t form x} and \eqref{Eq: Eq:mu_theta form x} into \eqref{Eq: L_t-1}, we get:} \\
L_{t-1} &=
\frac{1}{2\tilde{\beta}_t} ||\tilde{\vecmu}_t(\vecx_t, \vecx_0) - \vecmu_{\theta}(\vecx_t, t)||^2 + C \\
...
\textcolor{red}{Derive equation(14) in DDPM paper:} \\
