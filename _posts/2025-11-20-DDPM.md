...
\text{Transition densities }\quad &P(X_t​ \in dy∣X_s​=x) = q_{s,t​}(y|x)dy  \\
...
By Ito expansion:
$$
X_t = X_{t-h} + b(X_{t-h}, t-h)h + \sigma (W_t - W_{t-h}) + R_h
$$
where:
$$
\begin{align*}
& W_t - W_{t-h} \sim \mathcal{N}(0, h\vecI) \\
& R_h = o(h) \text{ in } L^2
\end{align*}
$$
Thus conditional on $X_{t-h}=y$,
$$
X_t = y + b(y, t-h)h + \sigma \sqrt{h}Z + o(h),
$$
where $Z \sim \mathcal{N}(0, \vecI)$. Therefore, 
$$
q_{t-h,t}(x|y)==\frac{1}{(2\pi \sigma^2 h)^{d/2}} exp\Big(−\frac{\lVert x−y−b(y,t−h)h\rVert^2}{2\sigma^2 h} \Big) +o(1)$$
uniformly on compact sets(Caveat, more rigorous). Next we use Bayes formula,
$$
q_{t,t-h}(y|x) = \frac{q_{t-h,t}(x|y)p_{t-h}(y)}{p_t(x)},
$$
Because variance is $O(h)$, the conditional mass concentrates near $y \approx x$, so define the reparameterization $y=x + \sqrt{h}z$.(Motivate this rigorously)

Now
$$
x−y−b(y,t−h)h=−\sqrt{h}​z−b(x,t)h+o(h)
$$
Then we square this and divide by $2\sigma^2 h$, then we have:
$$
q_{t-h,t}(x|y) \propto exp\Big(-\frac{z^2}{2\sigma^2}-\sqrt{h}\frac{z\cdot b}{\sigma^2}+O(h) \Big)
$$
Next we We expand We expand $p(y,t−h)$ around $(x,t)$ using joint Taylor expansion:
$$
\begin{align*}
p_{t-h}(y)&=p_t(x)+\nabla p_t(x)\cdot (y-x)-h\partial_t p_t(x)+O(|y-x|^2+h^2)  \\
&=p_t(x)+\sqrt{h}z\cdot \nabla p_t(x)+O(h)
\end{align*}
$$
where we assume the regularization on $p$ that $\partial_t p_t(x)$ is continuous, and hence locally bounded, can can be absorbed in $O(h)$. Then we divide above formula by $p_t(x)$ and exponentiate(using $1+\epsilon=e^{\epsilon+O(\epsilon^2)}$):
$$
\frac{p_{t-h}(y)}{p_t(x)}=exp\Big(\sqrt{h}z\cdot \nabla \log p_t(x)+O(h) \Big)
$$

Next, we multiply forward Gaussian term and density ratio, and complete the square, we can get the conditional distribution of $z$ given $x$
$$
z \sim \mathcal{N}\Big(\sqrt{h}\sigma^2\Big(\nabla \log p - \frac{b}{\sigma^2} \Big), \sigma^2 \vecI  \Big)
$$
Since $y=x+\sqrt{h}z$, then conditional distribution of $X_{t-h}$ given $X_t=x$ is $\mathcal{N}\Big(x+h(\sigma^2 \nabla \log p -b ), \sigma^2 h \vecI \Big)$, then we let $h \rightarrow \infty$ and get the reverse time SDE
$$
dX_t = (\sigma^2 \nabla \log p_t(X_t)-b(X_t, t))dt + \sigma d\bar{W}_t,
$$
where $\bar{W}_t$ is another Brownian motion.
{% endcapture %}
