...
where Const comes from the expectation on prior $p(\vecz_K)$. We can parameterize the distribution $q_0(\vecz_0\|\vecx)=\mathcal{N}(\vecmu(\vecx)\|\text{diag}(\vecsigma^2(\vecx)))$ parameters using a neural network, as well as the parameters $$\{\vecu_k,\, \vecpsi_k\}$$
