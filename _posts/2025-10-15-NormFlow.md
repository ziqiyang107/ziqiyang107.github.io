...
where Const comes from the expectation on prior $p(\vecz_K)$. We can parameterize the distribution $q_0(\vecz_0\|\vecx)=\mathcal{N}(\vecmu(\vecx)\|\text{diag}(\vecsigma^2(\vecx)))$ parameters using a neural network, as well as the parameters $$\{\vecu_k,\, \vecpsi_k\}$$. Now we are ready for the training of the normalizing flow:
1. Parameters: $\vecphi$ variational, $\vectheta$ generative
2. **while** not converged **do**
3. $\quad$ $\vecx$ $\leftarrow$ $$\{\text{a mini-batch}\}$$
4. $\quad$ $\vecz_0 \sim q_0(\cdot\|\vecx)$
7. $\quad$ $\vecz_K=f_K \circ f_{K-1} \circ...\circ f_1{\vecz_0}$
9. $\quad$ $\Delta \vectheta \propto \nabla_{\vectheta} L(\vecx, \vectheta, \vecphi)$
9. $\quad$ $\Delta \vecphi \propto \nabla_{\vecphi} L(\vecx, \vectheta, \vecphi)$
