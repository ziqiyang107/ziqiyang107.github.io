...
8. **end while**
During inference, we first sample from prior $\vecz_K \sim p(\vecz_K)=\mathcal(\vec0, \vecI)$, then sample from the trained likelihood/generative model $\vecx \sim p_{\vectheta}(\vecx\|\vecz)$
