...
&= \mathbb{E}_{q_0(\vecz_0|\vecx)}\big[\log q_0(\vecz_0|\vecx)-\sum_{k=1}^K \log |1+\vecu_k^T \psi_k(\vecz_{k-1})| - \log p_{\vectheta}(\vecx|\vecz_K) \big] + \textbf{Const},
\end{align*}
$$
</div>
where Const becomes from the expectation on prior $p(\vecz_K)$. 
