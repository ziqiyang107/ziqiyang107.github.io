...
where $$\psi_k(\vecz)=h'(\vecw_k^T \vecz + b)\vecw_k$$, and $h'$ denotes the derivative. We will quote authors' original text to state why this is called planar flows:
...
$$
\begin{align*}
-\text{ELBO}&=\mathbb{E}_{q_{\vecphi}(\vecz_K|\vecx)} \big[\log q_{\vecphi}(\vecz_K|\vecx) - \log p_{\vectheta}(\vecx, \vecz_K) \big] \\
...
where Const comes from the expectation on prior $p(\vecz_K)$. We can parameterize the distribution $q_0(\vecz_0\|\vecx)=\mathcal{N}(\vecmu(\vecx)\|\text{diag}\vecsigma^2(\vecx))$ using a neural network, as well as the parameters $\{\vecu_k,\, \vecpsi_k\}$
