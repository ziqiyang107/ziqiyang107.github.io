...
If we transform a random variable $\vecz \in \mathbb{R}^d$ using an invertible function $f: \mathbb{R}^d \rightarrow \mathbb{R}^d$ (a function called $f^{-1}$, such that $f^{-1} \odot f(\vecz)=\vecz$, then f is invertible and $f^{-1} is the inverse function of $f$), then $$\vecz'=f(\vecz)$$ has the distribution:
<div id="eq2">
$$
\begin{align*}
q(\vecz')=q(\vecz)\lVert\|\text{det}\frac{\partial f^{-1}}{\partial \vecz'} \rVert\| 
\end{align*}
$$
</div>
