...
$$
\log p_{\vecphi}(\vecx) = \log p_{prior} (\vecz) + \log \lVert \text{det}\frac{\partial f_{\vecphi}^{-1}(\vecx)}{\partial \vecx} \rVert
$$
...
$$
where each $\vecf_k$ is parameterized by a neural network. Now if we choose a planar flow:
...
$$
\log p_{\vecphi}(\vecx) &= \log p_{prior} (\vecx_0) + \sum_{k=0}^{L-1} \log \lVert \text{det}\frac{\partial \vecf_k}{\partial \vex_k} \rVert^{-1}
$$

where we have

$$
\log \lVert \text{det}\frac{\partial \vecf(\vecz)}{\partial \vecz} \rVert = \text{Tr}\big(\log(\vecI + \frac{\partial \vecv(\vecz)}{\partial \vecz}) \big),
$$

and this trace can be estimated via $\textbf{Hutchinson}$ $\textbf{trace}$ $\textbf{estimator}$. 
