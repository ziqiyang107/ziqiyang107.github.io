...
We have an unknown data distribution $p_{data}(x)$ by transforming a simple prior $p_{prior}(\vecz)$ (e.g., standard Gaussian $\mathcal{N}(\veczero, \vecI)$) via an invertible mapping:

$$
\vecf_{\vecphi}: \mathbb{R}^D \rightarrow \mathbb{R}^D
$$

with $\vecx = f_{\vecphi}(\vecz)$ and $\vecz \sim p_{prior}$. Here, $\vecx$ and $\vecz$ share the same dimension. Using the change of variables formula, the model likelihood is:
...
$$
L_{NF}(\vecphi) = \mathbb{E}_{\vecx \sim p_{data}} [\log p_{\vecphi}(\vecx)]
$$
...
$$
\vecf_{\vecphi} = \vecf_{L-1} \circ \vecf_{L-2} \circ ... \circ \vecf_0,
$$
...
where we have the Jacobian determinant:

$$

\left| 1 + \vecu^⊤ \vech^{′}(\vecw^⊤ \vecz + \vecb)\vecw \right|

$$

After we train the parameters in MLE framework, sampling from NFs is straightforward: draw $\vecx_0 \sim p_{prior}$ and compute $\vecx = \vecf_{\vecphi}(\vecx_0)$. 



---
{% include bibliography.html keys="rezende2015variational," %}
