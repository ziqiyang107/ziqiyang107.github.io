...
8. **end while**
During inference, we first sample from prior $\vecz_K \sim p(\vecz_K)=\mathcal(\veczero, \vecI)$, then sample from the trained likelihood/generative model $\vecx \sim p_{\vectheta}(\vecx\|\vecz)$. An annealed version of $-ELBO$ adds a $\beta_t=\min(1, 0.01+t/10000) \in [0,1]$ term in front of the $\log p_{\vectheta}(\vecx\|\vecz_K)$ term
