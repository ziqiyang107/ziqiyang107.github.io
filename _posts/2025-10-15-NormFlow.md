...
We have an unknown data distribution $p_{data}(x)$ by transforming a simple prior $p_{prior}(\vecz)$ (e.g., standard Gaussian $\mathcal{N}(0, \vecI)$) via an invertible mapping:

$$
\vecf_{\vecphi}: \mathbb{R}^D \rightarrow \mathbb{R}^D
$$

with $\vecx = f_{\vecphi}(\vecz)$ and $z \sim p_{prior}$. Here, x and z share the same dimension. Using the change-of-variables formula, the model likelihood is:

$$
\log p_{\vecphi}(\vecx) = \log p_{prior} (\vecz) + \log \left| \text{det}\frac{\partial f_{\vecphi}^{-1}(\vecx)}{\partial \vecx} \right|
$$

Parameters $\vecphi$ are learned by maximizing the likelihood over data:

$$
L_{NF}(\vecphi) = \mathbb{E}_{x \sim p_{data}} [\log p_{\vecphi}(\vecx)]
$$

But computing Jacobian in this formula is too expensive in practice. Normalizing flow came up with sequence of K trainable invertible mappings $$\{\vecf_k\}_{k=0}^{L-1}$$, each with efficiently computable Jacobians:

$$
\vecf_{\vecphi} = \vecf_{L−1} \circ f_{L−2} \circ ... \circ \vecf_0,
$$
where each $\vecf_k$ is parameterized by a neural network. Samples transform via

$$
\vecx_{k+1} = \vecf_k(\vecx_k),\, k = 0,..., L − 1,
$$
with $\vecz = \vecx_0 \sim p_{prior}$ and $\vecx = \vecx_L$, corresponding to data.

Now if we choose a planar flow:

$$
\vecf(\vecz) = \vecz + \vecu h(\vecw^⊤ \vecz + \vecb),
$$

We then have the simplified version:

$$
\log p_{\vecphi}(\vecx) = \log p_{prior} (\vecx_0) + \sum_{k=0}^{L-1} \log \left| \text{det}\frac{\partial \vecf_k}{\partial \vecx_k} \right|^{-1}
$$

where we have

$$
\log \left| \text{det}\frac{\partial \vecf(\vecz)}{\partial \vecz} \right| = \text{Tr}\big(\log(\vecI + \frac{\partial \vecv(\vecz)}{\partial \vecz}) \big),
$$

and this trace can be estimated via $\textbf{Hutchinson}$ $\textbf{trace}$ $\textbf{estimator}$. After we train the parameters in MLE framework, ampling from NFs is straightforward: draw $\vecx_0 \sim p_{prior}$ and compute $\vecx = \vecf \vecphi(\vecx_0)$. 
